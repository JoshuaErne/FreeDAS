...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
      ...
      self.names = custom_coco_class_name
  ...
  def add_custom_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    if self.names[cat] is not None:
      bbox = np.array(bbox, dtype=np.int32)
      # cat = (int(cat) + 1) % 80
      cat = int(cat)
      # print('cat', cat, self.names[cat])
  ...
  def add_coco_mask(self, bbox, cat, conf=1, img_id='default'): 
    bbox = np.array(bbox, dtype=np.int32)
    # cat = (int(cat) + 1) % 80
    cat = int(cat)
    # print('cat', cat, self.names[cat])
    ...
    cat_size = cv2.getTextSize(txt, font, 0.5, 2)[0]
    cv2.rectangle(
      self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255,255,255), -1)
...
custom_coco_class_name = [
     'person', 'bicycle', 'car', 'motorcycle', None,
     'bus', 'train', 'truck', 'boat', 'traffic light', 'fire hydrant',
     'stop sign', 'parking meter', None, None, 'cat', 'dog', 'horse',
     'sheep', 'cow', 'elephant', 'bear', 'zebra', 'giraffe', None,
